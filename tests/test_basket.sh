...
curl -s -X POST "http://localhost:8000/cart/add?product_id=1&quantity=2" \
  -H "Authorization: Bearer $TOKEN"
echo -e "\n"

echo "🛒 Добавляем товар product_id=2, quantity=1"
curl -s -X POST "http://localhost:8000/cart/add?product_id=2&quantity=1" \
  -H "Authorization: Bearer $TOKEN"
echo -e "\n"

# === Удаляем один товар ===
echo "🗑️ Удаляем товар product_id=1"
curl -s -X DELETE "http://localhost:8000/cart/remove?product_id=1" \
  -H "Authorization: Bearer $TOKEN"
echo -e "\n"

# === Проверяем корзину ===
echo "📦 Содержимое корзины:"
curl -s -X GET http://localhost:8000/cart/ \
  -H "Authorization: Bearer $TOKEN" | jq

# === Увеличиваем количество товара product_id=2 на 2 ===
echo "🔼 Увеличиваем количество товара product_id=2 на 2"
curl -s -X PATCH "http://localhost:8000/cart/increase?product_id=2&quantity=2" \
  -H "Authorization: Bearer $TOKEN"
echo -e "\n"

# === Уменьшаем количество товара product_id=2 на 1 ===
echo "🔽 Уменьшаем количество товара product_id=2 на 1"
curl -s -X PATCH "http://localhost:8000/cart/decrease?product_id=2&quantity=1" \
  -H "Authorization: Bearer $TOKEN"
echo -e "\n"
...
curl -s -X PATCH "http://localhost:8000/cart/decrease?product_id=2&quantity=5" \
  -H "Authorization: Bearer $TOKEN"
echo -e "\n"
...
curl -s -X POST "http://localhost:8000/cart/add?product_id=10&quantity=1" \
  -H "Authorization: Bearer $TOKEN"
echo -e "\n"
